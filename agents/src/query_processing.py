...
def answer_query_dense(query: str, embeddings, vectorstores, llm):
...
def answer_query_hybrid(query: str, embeddings, vectorstores, corpus, bm25, nlp, llm):
    merged_docs = hybrid_search(query, embeddings, vectorstores, corpus, bm25, nlp)
...
def generate_answer(llm, query: str, search_technique, embeddings, vectorstores, corpus, bm25, nlp):
    ...
    if mode == "semplice":
        ...
        if search_technique == "dense":
            answer, contexts = answer_query_dense(query, embeddings, vectorstores, llm)
        elif search_technique == "sparse":
            answer, contexts = answer_query_bm25(query, corpus, bm25, nlp, llm)
        elif search_technique == "hybrid":
            answer, contexts = answer_query_hybrid(query, embeddings, vectorstores, corpus, bm25, nlp, llm)
